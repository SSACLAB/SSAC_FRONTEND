...
// - img-src img.ssac.io: BE가 Cloudflare R2로 재업로드한 콘텐츠 이미지 CDN(커스텀 도메인)
...
const ContentSecurityPolicy = [
  ...
  "img-src 'self' data: blob: https://img.ssac.io",
  "font-src 'self' data:",
...
const nextConfig: NextConfig = {
  ...
  images: {
    remotePatterns: [
      // Cloudflare R2(커스텀 도메인) — 모든 콘텐츠 이미지 및 썸네일 CDN
      // BE(NotionImageMigrator)가 Notion S3 URL을 R2로 재업로드 후 영구 URL을 DB에 저장
      {
        protocol: 'https',
        hostname: 'img.ssac.io',
      },
    ],
  },
};
